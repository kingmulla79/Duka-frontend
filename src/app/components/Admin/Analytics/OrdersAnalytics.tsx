...
import { useGetOrderAnalyticsQuery } from "../../../../../redux/features/orders/orderAPI";
...
const OrderAnalytics: FC<Props> = ({ isDashboard }) => {
  ...
  const options = ChartCustomizations(
    theme || "",
    `New Order Count For The Last 12 Months`
  );

  defaults.font.family = "Poppins";

  const { data, error, isSuccess } = useGetOrderAnalyticsQuery(undefined, {
  ...
  const order_analytics: [] = data?.data;
  let labels: Array<string> = [];

  {
    order_analytics &&
      order_analytics.map((item: UserAnalyticsData, id: number) => {
    ...
    datasets: [
      {
        ...
        data:
          order_analytics &&
          order_analytics.map((item: UserAnalyticsData) => item.record_count),
